...
const loadQueryParams = (query) => {
  ...
  newPrice.value = '';
  // If productTitle or currentPrice missing, fetch from API
...
const fetchProductData = async () => {
  ...
  try {
    ...
    const variant = data.variants.find(v => String(v.id) === String(variantId.value));
    if (variant) {
...
// Watch for SPA route query changes
...
const updatePrice = async () => {
  ...
  try {
    const res = await fetch(`http://localhost:4000/api/products/${productId.value}`, {
      ...
      body: JSON.stringify({ variantId: variantId.value, newPrice: newPrice.value })
    });
          ...
          <div class="max-w-full mx-auto bg-white rounded-lg shadow border border-gray-200">
            ...
            <div class="px-6 py-4">
              <div class="mb-6 flex flex-col gap-1.5">
                ...
                <input v-model="newPrice" type="number" step="0.01"
              ...
              <div v-if="newPrice" class="bg-gray-50 p-4 rounded-lg mb-3">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between"> <span>Satış Fiyatı:</span><span class="font-medium">{{ newPrice }}
                      TL</span> </div>
                  <div class="flex justify-between text-red-600"> <span>Kargo Ücreti:</span><span>- {{ shippingCost }}
                      TL</span>
                  </div>
                  <div class="flex justify-between text-red-600"> <span>Komisyon (%{{ commissionRate }}):</span><span>-
                      {{
                      commissionAmount }} TL</span> </div>
                  <div class="flex justify-between border-t border-gray-200 pt-2 font-semibold"> <span>Net
                      Kazanç:</span><span class="text-green-600">{{ netProfit }} TL</span> </div>
                </div>
              </div>
            ...
            <div class="border-t border-gray-200 px-6 py-6 text-center text-sm text-gray-600">
             <div class="flex space-x-3 justify-end">
                <button @click="goBack"
                  class="px-4 py-2 rounded-md border border-[#E4E4E7] bg-[#F4F4F5] shadow-sm">
                  Geri Dön
                </button>
                <button @click="updatePrice" :disabled="!newPrice"
                  :class="['px-4 py-2 rounded-md bg-[#18181B] shadow ', newPrice ? 'px-4 py-2 rounded-md bg-[#18181B] shadow text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed']">
                  Fiyatı Güncelle
                </button>
              </div>
            </div>
